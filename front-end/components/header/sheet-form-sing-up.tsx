...
export function SheetFormSingUp({ open, onOpenChange }: SheetFormSingUpProps) {
  ...
  const isRepeatEmailValid =
    !Boolean(errors.repeatEmail) && getValues('repeatEmail').length > 0;
  const isFullNameValid =
    !Boolean(errors.fullName) && getValues('fullName').length > 0;
    ...
    >
      <SheetContent className="overflow-y-auto">
        ...
        <Form.Root onSubmit={handleSubmit(handleFormSingUpSUbmit)}>
          ...
          <Form.Control>
            <Form.Label htmlFor="email">
              Digite seu email{' '}
              <Form.FieldState isValid={isEmailsAreHitting && isEmailValid} />
            </Form.Label>
            <Form.Input
              id="email"
              className="placeholder-white/50"
          ...
          <Form.Control>
            <Form.Label htmlFor="repeatEmail">
              Repita seu email{' '}
              <Form.FieldState
                isValid={isEmailsAreHitting && isRepeatEmailValid}
              />
            </Form.Label>
            <Form.Input
              id="repeatEmail"
        ...
        <p className="text-sm text-white text-justify font-light">
          Ao clicar em “Criar conta”, certifico que tenho 16 anos ou mais e
          aceito os <span className="underline ">Termos de Uso</span>, as{' '}
          <span className="underline ">Políticas de Privacidade</span> e as{' '}
          <span className="underline ">Políticas de Cookies.</span>
        </p>
      </SheetContent>
    </Sheet>
  );
